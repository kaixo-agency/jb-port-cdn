...
$(document).ready(function () {
    var $cursor = $(".custom-cursor, .custom-cursor-backface"),
...
hoverLinks.forEach(link => {
  link.addEventListener('mouseenter', () => {
    // When hovering over a link, apply the inverted effect
    cursor.classList.add('hovering');
    link.style.color = 'transparent';  // Hide text color of the link temporarily
  });

  link.addEventListener('mouseleave', () => {
    // When mouse leaves, remove the inverted effect
    cursor.classList.remove('hovering');
    link.style.color = '';  // Restore original text color
  });
});

// CSS for inverted effect when the cursor is hovering over a link
document.addEventListener('mousemove', () => {
  if (cursor.classList.contains('hovering')) {
    // Dynamically apply filter on cursor element to invert the text inside its boundary
    cursorBg.style.filter = 'invert(1)';
  } else {
    // Remove the inversion when not hovering
    cursorBg.style.filter = '';
  }
});
